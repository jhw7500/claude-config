...
5. Before dispatch, set `BOUND_HEAD` to the snapshot HEAD, create `VIEW_ROOT` with `mktemp -d` and mode `0700`, set `VIEW_A`, `VIEW_B`, and `VIEW_C` beneath it, and run `git worktree add --detach "$VIEW_A" "$BOUND_HEAD"` plus the equivalent commands for B and C. Require each view's HEAD to equal `BOUND_HEAD`, its `git status --porcelain -uall` to be empty, and its `.review` to be absent. Start exactly three foreground/native reviewers in parallel (병렬), each with its dedicated cwd (`VIEW_A`, `VIEW_B`, or `VIEW_C`): Reviewer A, Reviewer B, and Reviewer C. On Claude use three native `Agent` calls. On Codex use three native `collaboration.spawn_agent` calls with separate task names, `fork_turns="none"`, self-contained messages, and the dedicated cwd. Give each one the shared snapshot, its role prompt, the strict report schema, and only its own projected prior context. Never provide a peer report.
6. Wait until all three reviewers are terminal, then make one bounded cleanup attempt for each view with `git worktree remove "$VIEW"` and remove `VIEW_ROOT` only if empty. Never use `--force`. Any failed, timed-out, or malformed response stops the entire run for user intervention after that cleanup attempt; this is non-pass, especially when a later round remains in progress. If cleanup refuses, also stop for user intervention. In either case do not fabricate or repair a report, do not reset verdict state, and do not delete `.review` state automatically.
