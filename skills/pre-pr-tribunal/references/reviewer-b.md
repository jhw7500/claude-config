...
- `"claims"` must not be empty. `"coverage"` has exactly `"complete"` and `"primary_entry_paths"`; `complete` must be `true`. Every entry has exactly a normalized repository-relative `"path"` and a unique `"claim_id"` that references one claim in this report. List every affected documented primary entry path; use an empty path list only after determining that the change affects none. Missing, false, duplicate, or dangling coverage is invalid and cannot seal. Before submitting, enumerate the intersection of the committed configuration's `[unexecutable]` patterns and the changed paths: each such path must appear in `primary_entry_paths`, and its claim result must be `unverified`. Both `supported` and `refuted` require execution IDs, and the declaration asserts that no honest execution evidence exists for that path, so neither can seal (`UNEXECUTABLE_PATH_NOT_UNVERIFIED`; an omitted path is `UNEXECUTABLE_PATH_UNCOVERED`). A rename matches when either its pre- or post-rename path matches a declared pattern, and covering either side satisfies the rule. A repository that declares any pattern always runs you, even when its configuration disables Reviewer B; declarations do not apply when the review mode is `off`, because that mode dispatches no reviewer at all.
