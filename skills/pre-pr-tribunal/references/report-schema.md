...
Report contract 4 requires Reviewer B to submit at least one claim plus `coverage: {"complete": true, "primary_entry_paths": [...]}`. Each primary-entry-path item has exactly `path` and `claim_id`; paths and claim IDs are unique, normalized, and every claim ID must resolve to a B claim in the same report. This makes the reviewer's completeness assertion and entry-path-to-claim mapping explicit before sealing. An empty path list is valid only when the reviewer found no affected documented primary entry path; the non-empty claim requirement still prevents an empty empirical authorization. Report contract 5 retains that shape and rejects a `supported` primary-entry-path claim whenever a cited execution yields a build dry-run that no cited successful execution matches with a live build of the same tool, working directory and target signature. Citing an unrelated non-build execution alongside the dry-run does not satisfy it. Machine rejection covers `make`, `gmake` and `ninja` only: a dry-run, plan or command-listing mode of any other build system is not detected, so for those the rule rests on reviewer discipline. Within the covered tools the classifier fails closed on an option-carrying environment assignment (`MAKEFLAGS`, `MFLAGS`, `GNUMAKEFLAGS`) whose value it cannot statically resolve. It looks inside another command only for executables documented to take a command string - a shell with `-c`, `ssh`, `nix-shell --run`, `su -c` - because a quoted operand of anything else is data, such as a search pattern or a message, and must not be read as a command; a dry run nested in any other wrapper is not detected and rests on reviewer discipline. Shell redirection is removed before the signature is computed, so `> log`, `>> log`, `2> log` and `2>&1` never change which build an execution records, and a command substitution is dropped, so a build appearing only inside `$( ... )` mints no evidence at all and a build option supplied through one is not modeled. Rejected evidence must remain `unverified`, making finalization inconclusive instead of PASS.
