...
- A finding has exactly `"id"`, `"reviewer"`, `"severity"`, `"title"`, `"rationale"`, `"path"`, `"line"`, `"execution_ids"`, and `"acceptance_condition"`. A finding path may be any normalized repository-relative path, including outside the diff. The rationale names removable complexity and the acceptance condition states behavior the smaller form preserves. This does not authorize writes: only controller automatic fixes are limited to round-1 `initial_paths`.
