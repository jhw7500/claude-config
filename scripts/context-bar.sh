...
# Count active background tasks (managed by bg-task-progress-hook.py)
# Location: ~/.claude/state/bg-tasks/ (user-scoped; avoids /tmp world-writable issue)
# Only count tasks for the CURRENT session (derived from session_id in input JSON),
# falling back to transcript filename if session_id absent.
# Filter: 4h mmin window to ignore orphans from crashed sessions / missed Post events.
bg_tasks=""
bg_root="${HOME}/.claude/state/bg-tasks"
if [[ -d "$bg_root" ]]; then
    session_id=$(echo "$input" | jq -r '.session_id // .sessionId // empty')
    if [[ -z "$session_id" && -n "$transcript_path" ]]; then
        # Fallback: derive a stable key from transcript filename
        session_id=$(basename "$transcript_path" .jsonl 2>/dev/null)
    fi

    if [[ -n "$session_id" && -d "$bg_root/$session_id" ]]; then
        # Current-session scope
        bg_count=$(find "$bg_root/$session_id" -maxdepth 1 -type f -name '*.task' -mmin -240 2>/dev/null | wc -l | tr -d ' ')
    else
        # No session identifier available → show global count (legacy behavior)
        bg_count=$(find "$bg_root" -maxdepth 2 -type f -name '*.task' -mmin -240 2>/dev/null | wc -l | tr -d ' ')
    fi

    if [[ "$bg_count" -gt 0 ]]; then
        bg_tasks=" | 🔄 BG:${bg_count}"
    fi

    # Opportunistic cleanup across all sessions: remove .task files older than 24h
    find "$bg_root" -maxdepth 2 -type f -name '*.task' -mmin +1440 -delete 2>/dev/null
fi

# Build output: user@host | Model | Dir | Branch (uncommitted) | BG tasks | Context
output="${C_GRAY}${user_host}${C_RESET} ${C_ACCENT}${model}${C_GRAY} | 📁${dir}"
