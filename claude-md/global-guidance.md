...
이 저장소는 같은 계약을 이미 갖고 있다 — `skills/superpowers-extras/SKILL.md`가 "양성 대조가
없으면 *차단됨*과 *도달한 적 없음*은 구별되지 않는다"를 명시하고(267행), 감시·폴링·필터의 침묵을
믿기 전에 양성 대조를 요구하며(354행), 증거 표(426행)와 Common Failures(439행)에 같은 실패를
올려 둔다. 이 절은 그 계약을 계측·실기 쪽으로 확장한 것이다. **둘이 어긋나면 그 파일이 정본이고,
이 절을 고칠 때 그 파일도 같이 본다.** 스냅샷 밖인 것은 이 일반화를 촉발한 외부 세션 기록뿐이며,
규칙의 근거는 위 메커니즘이다.
