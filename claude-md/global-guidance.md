...
9. **Task 등록 권유 (넛지)** — PreToolUse 훅의 `[TASK-NUDGE]` 리마인더(세션 첫 프로젝트 파일 수정 직전, 세션당 1회)를 받으면: 이 세션에서 아직 task start를 하지 않았고, cwd가 Project Control 포트폴리오 등록 저장소이며, 실질 작업(코드 수정·기능 변경·디버깅)이면 **작업 착수 전에 AskUserQuestion으로 1회 권유**한다 — ① Formal Issue Task(기존 Issue URL 또는 `gh issue create` 후 start) ② Temporary Task(goal/done/scope는 대화의 요청문에서 도출해 확인 후 start) ③ Task 없이 진행. 등록 선택 시 `/jhw:task` 규격을 유지하며 `"$HOME/.local/bin/jhw-control-host" preflight`가 성공한 뒤, 신규 Formal/Temporary Task는 현재 checkout의 exact root와 `"$HOME/.local/bin/jhw-control-host" task start --resolve-from-checkout true`를 사용한다. 기존 Task 재개는 `"$HOME/.local/bin/jhw-control-host" task start --task <tsk-id>`를 사용한다. Project/Repository ID를 추측하거나 portfolio pagination으로 조합하지 않는다. finish/switch도 `"$HOME/.local/bin/jhw-control-host" task finish`를 사용한다. `PROJECT_REPOSITORY_NOT_FOUND`는 정확한 Project Record에 Repository를 등록한 뒤 재시도하고, `PROJECT_REPOSITORY_AMBIGUOUS`는 Repository 연관을 하나로 축소한 뒤 재시도한다. 어느 경우에도 Project를 임의 선택하거나 explicit mode로 자동 fallback하지 않는다. preflight가 저장소 잠김을 보고하면 사용자 터미널에서 `"$HOME/.local/bin/jhw-control-host" unlock` 한 번만 실행한 뒤 preflight를 재시도한다. raw CLI 호출·설정 소싱·직접 자격증명 조회·daemon 교체는 금지한다. 조회·Q&A·문서/설정만 만지는 세션과 서브에이전트는 권유 없이 진행하고, "Task 없이"를 선택한 세션에서는 다시 묻지 않는다.
