...
이 파일은 사용자 홈(`~/.claude/CLAUDE.md`)에 위치하여 **모든 프로젝트 세션에 전역 적용**된다. 프로젝트별 지침은 각 프로젝트의 `CLAUDE.md` / `AGENTS.md`에 둔다.

> OMC 설정은 `CLAUDE-omc.md` 참조 (file-split pattern). 아래 사용자 지침이 OMC 기본 동작보다 우선한다.

---

> Notion 작업 전용 지침은 `@CLAUDE-notion.md` 참조 — `/jhw:*` 스킬과 `mcp__notion__*` / `mcp__jhw-notion__*` 도구 호출 시 적용.
...
**위반 사례 (2026-04-29)**:
- "끝나면 자동 알림" 약속 후 사용자가 "결과 보고해줘" 입력해야 보고 → 반복 발생, 사용자 불만.
