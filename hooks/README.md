...
Command-local PATH는 exact `PATH=/usr/bin:/bin` 하나이고 executable은 literal `/usr/bin/gh`여야
한다. PATH 바인딩 누락·중복·변형, bare/다른 path, execution wrapper와 `LD_*`, `DYLD_*`
override는 `COMMAND_AMBIGUOUS`로 fail closed한다.
